...
`akvps` 是在单台 VPS 内运行的中文初始设置工具。第一版重点做三件事：把当前 VPS 准备成可接入主 3x-ui 的远端节点端；简单部署一台 3x-ui 主控面板；部署 Komari 探针被控端。
...
- 完整实现：初始设置、主控面板部署、Komari 探针部署。
- 占位菜单：接入信息、修复工具、设置 / 更新 / 卸载。
- 支持系统：Debian 11/12/13，Ubuntu 22.04/24.04。
- 不做本地总控、不做多 VPS 台账、不部署 Komari 面板、不做 Halo、不做成本统计。
