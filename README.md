...
`akvps` 是在单台 VPS 内运行的中文节点部署工具。第一版重点做：远端节点部署、3x-ui 面板部署、Komari 探针部署、Komari 主站部署。
...
- `节点面板`：把当前 VPS 部署成可接入主控的 3x-ui 远端节点。
- `主控面板`：把当前 VPS 部署成 3x-ui 主控面板。
...
第一版面向新 VPS 或可覆盖配置的 VPS。重复运行节点部署或面板部署会刷新 3x-ui 用户名、密码、WebBasePath 和 API Token，但保留已有 3x-ui 数据库、入站和客户端。
