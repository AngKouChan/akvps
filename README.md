...
- 按内存判断是否创建 Swap。
- 安装必要工具并设置上海时区。
- 默认使用稳妥调优，也可选择进阶网络调优、基础调优或 BBRv3；BBRv3 使用 Eric86777/vps-tcp-tune，首次会先安装内核并暂停，重启 VPS 后再继续部署后续服务。
- 配置 UFW。
...
`更新 / 卸载 -> 清除所有设定` 只清除 akvps 自己保存的配置、密钥、角色状态和报告，不删除 3x-ui、Komari，也不改 SSH / UFW。
